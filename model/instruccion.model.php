...
class InstruccionModel
{
    ...
    public function creatNewInstruccion($data, $dirc_file, $new_nom_archivo)
    {
        try {
            ...
            $sql = "INSERT INTO instruccion ( instru_titulo, instru_descripcion, instru_file, instru_nomarchivo, clien_id, sed_id, usua_id, instru_fecha) VALUES (?,?,?,?,?,?,?,?)";
    ...
    public function creatNewInstruccionTodasSedes($data, $dirc_file, $new_nom_archivo)
    {
        try {
            session_start();
            $modelNovedad = new NovedadModel();
            $sedes = $modelNovedad->selectSedes();
            session_start();
            $idsession = $_SESSION["idusuario"];
            $idcliente = $_SESSION["idcliente"];
            date_default_timezone_set('America/Bogota');
            ...
            foreach ($sedes as $sede) {
                $sql = "INSERT INTO instruccion ( instru_titulo, instru_descripcion, instru_file, instru_nomarchivo, clien_id, sed_id, usua_id, instru_fecha) VALUES (?,?,?,?,?,?,?,?)";
                $query = $this->pdo->prepare($sql);
                $save = $query->execute(array($data[1], $data[2], $dirc_file, $new_nom_archivo, $idcliente, $sede['sed_id'], $idsession, $fechayhora));
                $arr = $query->errorInfo();
                $id = $this->pdo->lastInsertId();
                if ($save == true) {
                    $sql = 'SELECT usua_id FROM usuario WHERE clien_id = ?';
                    ...
                    if (count($result) == 0 && $const == 1) {//Sin no arrojó resultados && se ejecutó
                        ...
                        foreach ($result as $key => $value) {
                            $sql1 = "INSERT INTO notificacionins (notins_notificacion,notins_leido, notins_usuario, clien_id) VALUES (?,?,?,?)";// voy aqui
                            $query1 = $this->pdo->prepare($sql1);
                            $save1 = $query1->execute(array($id, 0, $value["usua_id"], $idcliente));
                        }
                        if ($save == true && $save1 == true) {
                            continue;
                        } else {
    ...
    public function createInstruccionManualTodasSedes($data)
    {
        try {
            session_start();
            $idcliente = $_SESSION["idcliente"];
            ...
            $conver_fecha = date("Y-m-d H:i:s", strtotime($data[2])); //Configuro el formato de hora
            $modelNovedad = new NovedadModel();
            $sedes = $modelNovedad->selectSedes2();
            $save = false;
            $save1 = false;
            foreach ($sedes as $sede) {
                $sql = "INSERT INTO instruccionmanual (instruman_descripcion, instruman_fecha, clien_id, sed_id, usua_id) VALUES (?,?,?,?,?)";
                $query = $this->pdo->prepare($sql);
                $save = $query->execute(array($data[1], $conver_fecha, $idcliente, $sede['sed_id'], $idusuario));
                $id = $this->pdo->lastInsertId();
                $arr = $query->errorInfo();
                if ($save == true) {
                    $sql = 'SELECT usua_id FROM usuario WHERE clien_id = ?';
                    ...
                    if (count($result) == 0 && $const == 1) {//Sin no arrojó resultados && se ejecutó
                        ...
                        foreach ($result as $key => $value) {
                            $sql1 = "INSERT INTO notificacioninsm (notinsm_notificacion, notinsm_leido,notinsm_usuario, clien_id) VALUES (?,?,?,?)";// voy aqui
                            $query1 = $this->pdo->prepare($sql1);
                            $save1 = $query1->execute(array($id, 0,$value["usua_id"], $idcliente));
                            $arr = $query1->errorInfo();
                        }
                        if ($save == true && $save1 == true) {
                            continue;
                        } else {
