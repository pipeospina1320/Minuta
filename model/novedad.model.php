...
class NovedadModel{
   ...
   public function createNovedades($novedad, $dirc_file, $new_nom_archivo){
     try {
       session_start();
       ...
       $foto = 1;
       $sql = "INSERT INTO novedad (nove_id, clien_id, sed_id, servi_id, nove_turno , nove_novedad, nove_fecha, nove_fechareal, usua_id, tn_id, nove_file, nove_nomarchivo, nove_foto) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
   ...
   // Funcion para crear una novedad
   public function createNovedad($novedad){
     try {
       session_start();
       $idsession = $_SESSION["idusuario"];
       $idcliente =   $_SESSION["idcliente"];
       $conver_fecha = date("Y-m-d H:i:s", strtotime($novedad[5])); //Configuro el formato de hora
       $conver_fechareal = date("Y-m-d H:i:s", strtotime($novedad[6])); //Configuro el formato de hora
       $sql = "INSERT INTO novedad (nove_id, clien_id, sed_id, servi_id, nove_turno , nove_novedad, nove_fecha, nove_fechareal, usua_id, tn_id) VALUES (?,?,?,?,?,?,?,?,?,?)";
       $query = $this->pdo->prepare($sql);
       $save = $query->execute(array('', $idcliente, $novedad[0], $novedad[1], $novedad[2], $novedad[4], $conver_fecha, $conver_fechareal, $idsession, $novedad[3]));
        // var_dump($save);
        if ($save == 1) {
         return true;
         }else {
